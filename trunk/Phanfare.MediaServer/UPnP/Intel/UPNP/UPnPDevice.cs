...
namespace Intel.UPNP
{
    ...
    public sealed class UPnPDevice
    {
        ...
        private HTTPMessage Get(string GetWhat, IPEndPoint local, HTTPMessage msg, HTTPSession WebSession)
        {
			...
			if( ( GetWhat == "icon.jpg" ) && ( this._icon != null ) )
			{
				lock( this._icon )
				{
					MemoryStream stream = new MemoryStream();
					this._icon.Save( stream, ImageFormat.Jpeg );
					message.StatusCode = 200;
					message.StatusData = "OK";
			...
			if( ( GetWhat == "icon.bmp" ) && ( this._icon != null ) )
			{
				lock( this._icon )
				{
					MemoryStream stream = new MemoryStream();
					this._icon.Save( stream, ImageFormat.Bmp );
					message.StatusCode = 200;
					message.StatusData = "OK";
					message.ContentType = "image/bmp";
					message.BodyBuffer = stream.ToArray();
					stream.Close();
				}
				return message;
			}
			if( ( GetWhat == "icon2.jpg" ) && ( this._icon != null ) )
			{
				// Hrm
				Console.WriteLine( "UPnPDevice::Get: request for icon2 - not yet implemented" );
			}
            bool flag = false;
            for (int i = 0; i < this.Services.Length; i++)
            {
            ...
            try
            {
                ...
                if (GetWhat.ToLower().StartsWith("pictures") == true
					...
					|| GetWhat.ToLower().StartsWith( "photos" ) == true
        ...
        private void GetNonRootDeviceXML(IPEndPoint local, XmlTextWriter XDoc)
        {
            ...
            if (this._icon != null)
            {
                lock (this._icon)
                {
                    ...
                    XDoc.WriteEndElement();
                    XDoc.WriteStartElement("icon");
                    XDoc.WriteElementString("mimetype", "image/jpeg");
                    XDoc.WriteElementString("width", this._icon.Width.ToString());
                    XDoc.WriteElementString("height", this._icon.Height.ToString());
                    XDoc.WriteElementString("depth", Image.GetPixelFormatSize(this._icon.PixelFormat).ToString());
                    ...
                    XDoc.WriteEndElement();
					XDoc.WriteStartElement( "icon" );
					XDoc.WriteElementString( "mimetype", "image/bmp" );
					XDoc.WriteElementString( "width", this._icon.Width.ToString() );
					XDoc.WriteElementString( "height", this._icon.Height.ToString() );
					XDoc.WriteElementString( "depth", Image.GetPixelFormatSize( this._icon.PixelFormat ).ToString() );
					XDoc.WriteElementString( "url", "/icon.bmp" );
					XDoc.WriteEndElement();
                    if (this._icon2 != null)
                    {
                        XDoc.WriteStartElement("icon");
						...
						XDoc.WriteElementString( "height", this._icon2.Height.ToString() );
						XDoc.WriteElementString( "depth", Image.GetPixelFormatSize( this._icon2.PixelFormat ).ToString() );
						XDoc.WriteElementString( "url", "/icon2.bmp" );
						XDoc.WriteEndElement();
                    }
                    XDoc.WriteEndElement();
                }
			...
			//MOD
			//XDoc.WriteElementString( "dlna:X_DLNADOC", "DMS-1.00" );
			XDoc.WriteElementString( "X_DLNADOC", "dlna", "DMS-1.00" );
		...
		//MOD
		// Not thread safe, or smart...
		private IPEndPoint _lastEndPoint;
		public IPEndPoint LastEndPoint { get { return _lastEndPoint; } }
        ...
        private void HandleWebRequest(HTTPMessage msg, HTTPSession WebSession)
        {
            ...
            else
            {
				...
				if( directive == "POST" )
                {
                    try
                    {
						_lastEndPoint = WebSession.Source;
                        packet = this.Post(directiveObj, msg.StringBuffer, msg.GetTag("SOAPACTION"), msg, WebSession);
                    }
        ...
        private HTTPMessage Invoke(string Control, string XML, string SOAPACTION, HTTPMessage msg, HTTPSession WebSession)
        {
            string actionName = "";
        ...
        private HTTPMessage Post(string MethodData, string XML, string SOAPACTION, HTTPMessage msg, HTTPSession WebSession)
        {
            return this.Invoke(MethodData.Substring(1), XML, SOAPACTION, msg, WebSession);
        }
